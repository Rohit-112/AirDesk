...
class SessionRepositoryImpl @Inject constructor() : ISessionRepository {
    ...
    override fun removeListener(code: String, node: String, listener: ValueEventListener) {
        database.child(code).child(node).removeEventListener(listener)
    }

    override fun writeClipboard(code: String, node: String, text: String) {
        database.child(code).child(node).setValue(text)
    }

    override fun deleteSession(code: String) {
